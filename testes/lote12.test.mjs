...
await ok("Aluno com código marca presença normal", marcarAluno(anon, presenca()));
await nega("Presença SEM expiraEm (nunca seria apagada)", marcarAluno(anon, (({ expiraEm, ...r }) => r)(presenca())));
await nega("Presença com expiraEm em 2099", marcarAluno(anon, presenca({ expiraEm: Timestamp.fromMillis(Date.parse("2099-01-01")) })));
await nega("Presença com campo extra de 500 KB", marcarAluno(anon, presenca({ lixo: "x".repeat(500000) })));
await nega("Presença com campo extra pequeno", marcarAluno(anon, presenca({ extra: 1 })));
await nega("Presença com nome de 200 caracteres", marcarAluno(anon, presenca({ nome: "A".repeat(200) })));
await nega("Presença com nome vazio", marcarAluno(anon, presenca({ nome: "" })));
await nega("Presença com data fora do formato", marcarAluno(anon, presenca({ data: "hoje" })));
await nega("Presença com horário gigante", marcarAluno(anon, presenca({ horario: "x".repeat(50) })));
await nega("Presença com aparelho gigante", marcarAluno(anon, presenca({ maquina: "x".repeat(100) })));
await nega("Presença com código errado", marcarAluno(anon, presenca({ codigoUsado: "0000" })));
