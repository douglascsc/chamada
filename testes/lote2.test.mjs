...
check("Chamada: botão mostra quantas ocorrências", /^Copiar ocorrências \(\d+\)$/.test(await page.textContent("#btn-bar-copy-absent")), await page.textContent("#btn-bar-copy-absent"));
await page.click("#btn-bar-copy-absent"); await page.waitForTimeout(300);
const txt = await clip(page);
const linhasOc = txt.split("\n").slice(1);
check("Copiar ocorrências (chamada): título + um aluno por linha em ordem alfabética", txt.startsWith("Ocorrências - INF2M 2026 - Banco de Dados - ") && txt.includes("Bruno Henrique Alves - Ausente") && !txt.includes("Camila Ferreira - Ausente") && linhasOc.every((l, i) => i === 0 || linhasOc[i - 1].localeCompare(l, "pt-BR") <= 0), JSON.stringify(txt.split("\n")));
...
check("Copiar ocorrências (histórico, dia mais recente)", hist.startsWith("Ocorrências - INF2M 2026 - Banco de Dados - ") && hist.includes(" - Ausente"), hist.split("\n")[0]);
...
check("Exportar 7 dias: uma coluna por dia + Presenças + Faltas + Atrasos + Saídas antecipadas", header.length === 8 && header[0] === "Aluno" && header[4] === "Presenças" && header[5] === "Faltas" && header[6] === "Atrasos" && header[7] === "Saídas antecipadas", header.join(" | "));
